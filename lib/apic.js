...
define(function (require) {
    ...
    return function apic(desc, options) {
        ...
        root.anonimize = function() {
            options.cookie = false;
        };

        root.deanonimize = function() {
            options.cookie = true;
        };
        ...
        function method(verb, uri, res, params, secure) {
            var safe = safeMethods[verb] || false,
                protocol = options.ssl || (secure = secure || !safe) ? 'https:' : 'http:';
            ...
            return function () {
                ...
                http.request({
                    ...
                    if (err) {
                        if (err.status === 401 && root.authorize()) {
                            root.unauthorize();
                        }
                    ...
                    res.variable && (variables[res.variable] = res[body]);
