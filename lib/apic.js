define(function (require) {
	...
	return function apic(desc, ssl, options) {
		...
		function method(verb, uri, res, params, secure) {
			...
			return function () {
				...
				params.forEach(function (name, i) {
					if (query[name] !== undefined && args[i] !== undefined)
				...
				return request(verb, protocol + base + path, query, headers)
		...
		return function generate(map, url, res) {
			var key,
				value,
				parts,
				secure,
				...
				if (map.hasOwnProperty(key) && key !== '@') {
					...
					if (key.charAt(0) !== ':') {
						...
						tree = typeof value === 'string'
							...
							: generate(value, (url || '') + '/' + (value['@'] || key), transparent && res);
