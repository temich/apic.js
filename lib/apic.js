		...
		function method(verb, uri, result, params, secure) {
			...
			return function () {
				...
				if (params.reduce(function (a, b) {
					return a - +query.hasOwnProperty(b.name);
				...
				request(verb, protocol + base + path, query, headers)
					...
					.always(function (a, status, b) {
						var xhr = status === 'success' ? b : a, // WTF
							tok = xhr.getResponseHeader(tokenHeader);

						secure && tok && token(xhr.getResponseHeader(tokenHeader));
					});
			};
		}
