define(function (require) {
	...
	return function apic(desc, ssl, options) {
		...
		var base = desc.base,
			...
			representMethods = { 'POST': true, 'PUT': true },
		...
		function method(verb, uri, res, params, secure) {
			var safe = safeMethods[verb] || false,
				needRepresnt = representMethods[verb] || false,
				protocol = ssl || (secure = secure || !safe) ? 'https:' : 'http:';

			return function () {
				var args = Array.prototype.slice.call(arguments),
					callback = last(args) === 'function' ? args.pop() : undefined,
					representation = (needRepresnt && last(args) === 'object') ? args.pop() : undefined,
				...
				if (params.reduce(function (a, b) {
					return a - +((variables[b.variable] !== undefined) || query[b] !== undefined);
